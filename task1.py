...
class Birthday(Field):
    ...
    @value.setter        
    def value(self, v):
        if re.search("^(0[1-9]|[1,2][0-9]|3[0-1])\.(0[1-9]|1[0-2])\.(19\d\d|20\d\d)$",v): 
            self.__value = v
        else:
            raise ValueError 
...
def get_birthdays_per_week(args, book):
    ...
    users=[{"name": key.value, "birthday": datetime.strptime(value.birthday.value, "%d.%m.%Y")} for key,value in book.items() if 'birthday' in value.__dict__]
    res={}
